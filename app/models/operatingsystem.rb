...
class Operatingsystem < ActiveRecord::Base
  ...
  validates :major, :presence => { :message => "Operating System version is required" }
  ...
  validates :major, :numericality => true
  validates :minor, :numericality => true, :allow_nil => true, :allow_blank => true
  validates :name, :format => {:with => /\A(\S+)\Z/, :message => "can't be blank or contain white spaces."}
  ...
  audited
  default_scope lambda { order('LOWER(operatingsystems.name)') }
