module Classification
  class Base
    ...
    def puppetclass_ids
      ...
      ids += host.group_puppetclass_ids unless hostgroup
      if hostgroup
        ids += HostgroupClass.where(:hostgroup_id => hostgroup.path_ids).pluck(:puppetclass_id)
        ids += hostgroup.group_puppetclass_ids
        HostConfigGroup.where(['(host_id = ? AND host_type = ?) OR (host_id = ? AND host_type = ? )', host.id, 'Host::Base', hostgroup.id, 'Hostgroup'])
      end
