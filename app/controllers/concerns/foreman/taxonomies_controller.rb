module Foreman::TaxonomiesController
  ...
  def import_mismatches
    ...
    if @taxonomy
      ...
      redirect_to send("edit_#{taxonomy_single}_path", @taxonomy), :notice => _("All mismatches between hosts and #{@taxonomy.name} have been fixed")
    else
  ...
  def assign_all_hosts
    ...
    redirect_to send("#{taxonomies_plural}_path"), :notice => _("All hosts previously with no #{taxonomy_single} are now assigned to #{@taxonomy.name}")
  end
  ...
  def assign_selected_hosts
    ...
    @taxonomy.import_missing_ids
    redirect_to send("#{taxonomies_plural}_path"), :notice => _("Selected hosts are now assigned to #{@taxonomy.name}")
  end
