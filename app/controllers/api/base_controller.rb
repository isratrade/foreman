module Api
  ...
  class BaseController < ActionController::Base
        ...
        if param =~ /(\w+)_id$/
          if allowed_nested_id.include?(param)
            resource_identifying_attributes.each do |key|
              ...
              @nested_obj ||= klass.constantize.send(find_method, params[param])
            end
          else
            ...
            # unless it is explictedly declared in skip_nested_id in the case where there is 2-level nesting . Ex. puppetclasses/apache/smart_variables/4/override_values
    ...
    def get_klass_name(klass)
      klass = "lookup_key" if ['smart_variable', 'smart_class_parameter'].include?(klass)
      klass.classify
    end
