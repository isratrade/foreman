...
#    assert_response :unprocessable_entity
  ...
  test "no root name on index if param root_name is 'false'" do
    get :index, {:root_name => "false" }
    response = ActiveSupport::JSON.decode(@response.body)
    refute response.kind_of?(Hash)
    assert response.kind_of?(Array)
    assert response[0].kind_of?(Hash)
    assert_equal ['id', 'name'], response[0].keys.sort
  end
