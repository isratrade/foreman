...
class Api::V2::EnvironmentsControllerTest < ActionController::TestCase
  ...
  test "should get index" do
    ...
    assert !envs['environments'].empty?
    assert_equal 3, envs['environments'].count
  end

  test "should get environments for nested object" do
    get :index, {:puppetclass_id => puppetclasses(:one) }
    assert_response :success
    assert_not_nil assigns(:environments)
    envs = ActiveSupport::JSON.decode(@response.body)
    assert !envs.empty?
    assert !envs['environments'].empty?
    assert_equal 1, envs['environments'].count
  end
