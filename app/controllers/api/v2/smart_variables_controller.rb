module Api
  module V2
    class SmartVariablesController < V2::BaseController
      ...
      #TODO = search_for caused not to return results on nesting so I deleted it.  put it back
      def index
        if nested_obj
          if nested_obj.kind_of?(Puppetclass)
            @smart_variables = LookupKey.global_parameters_for_class(nested_obj.id).paginate(paginate_options)
            logger.info "GOOTTTT HEREE #{nested_obj.id}"
            logger.info @smart_variables.inspect
          else
            puppetclass_ids = nested_obj.all_puppetclasses.map(&:id)
            @smart_variables = LookupKey.global_parameters_for_class(puppetclass_ids).paginate(paginate_options)
          end
        else
      ...
      # only smart variables (not smart class parameters) are manually deleted by API, similar to create
      def destroy
        @smart_variable = @lookup_key
